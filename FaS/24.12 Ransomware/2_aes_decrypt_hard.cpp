...
void aes_dec(BYTE* plaintext,BYTE* ciphertext, int ciphertext_size, BYTE* key, int key_size, BYTE* iv) {
	...
	BCRYPT_ALG_HANDLE dec_handle = NULL;
	status = BCryptOpenAlgorithmProvider(&dec_handle, BCRYPT_AES_ALGORITHM, NULL, 0);
	...
	//키 핸들 / ciphertext / ciphertext 크기 / - / IV / IV 크기 / NULL로 설정해야 ciphertext 크기 계산 / - / 버퍼 주소 / 패딩 지정 플래그
	...
	//6. BCryptDecrypt: 복호화
	//키 핸들 / ciphertext / ciphertext 크기 / - / IV / IV 크기 / plaintext 받을 버퍼 주소 / 버퍼 크기 / 복사된 값 크기 / 패딩 지정 플래그
